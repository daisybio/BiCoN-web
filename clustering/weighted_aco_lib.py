...
def ants_manager(a, b, n, m, H, GE, G, clusters, cost_limit, K, evaporation, th, L_g_min, L_g_max, eps, times,
                 session_id, n_proc, opt=None, pts=False, show_pher=True, show_plot=True, save=None, show_nets=False):
    if (int(n_proc) == 1):
        return (
            ants_new(a, b, n, m, H, GE, G, clusters, cost_limit, K, evaporation, th, L_g_min, L_g_max, eps, times, opt,
                     pts, show_pher, show_plot, save, show_nets))
    if (int(n_proc) > 1) and (int(n_proc) <= multiprocessing.cpu_count()):
        print("cpu count " + str(multiprocessing.cpu_count()))
        return (
            ants_paral_new(a, b, n, m, H, GE, G, clusters, cost_limit, K, evaporation, th, L_g_min, L_g_max, eps, times,
                           n_proc, opt, pts, show_pher, show_plot, save, show_nets))
    else:
        raise Exception(
            'n_proc should not exceed {0}. The value of x was: {1}'.format(multiprocessing.cpu_count(), n_proc))


def ant_job_paral_OLD(GE, N, H, th, clusters, probs, a, b, cost, m, n, patients, count_big, cost_limit, L_g_min,
    ...
    for i in range(ants_per_batch):
        seed = ss[i]
        tot_score, gene_groups, patients_groups, new_scores, wars, no_int = ant_job(GE, N, H, th, clusters, probs, a, b,
                                                                                    cost, m, n, patients, count_big,
                                                                                    cost_limit, L_g_min, L_g_max, G, ge,
                                                                                    seed)
        W = W + wars
...
def ant_job_paral(GE, N, H, th, clusters, probs, a, b, cost, m, n, patients, count_big, cost_limit, L_g_min, L_g_max, G,
                  ge, ants_per_batch, pr, ss):
    max_round_score = -100
    W = 0
    av_score = 0
    for i in range(ants_per_batch):
...
def ants_new(a, b, n, m, H, GE, G, clusters, cost_limit, K, evaporation, th, L_g_min, L_g_max, eps, times, session_id,
             opt=None, pts=False, show_pher=True, show_plot=True, print_runs=True, save=None, show_nets=True):
    ge = GE.values
    H = H.astype(np.short)
    ...
    while np.abs(max_round_score - av_score) > eps and count_big < times and (W < m / 3):
        ...
        for i in range(K):
            # for each ant
            st = time.time()
            tot_score, gene_groups, patients_groups, new_scores, wars, no_int = ant_job(GE, N, H, th, clusters, probs,
                                                                                        a, b, cost, m, n, patients,
                                                                                        count_big, i, cost_limit,
                                                                                        L_g_min, L_g_max, G, ge,
                                                                                        print_runs)
            end = time.time()
            W = W + wars
...
def ants_paral_new(a, b, n, m, H, GE, G, clusters, cost_limit, K, evaporation, th, L_g_min, L_g_max, eps, times,
                   session_id, opt=None, pts=False, show_pher=True, show_plot=True, print_runs=True, save=None,
                   show_nets=True):
    ge = GE.values
    H = H.astype(np.short)
    N = neigborhood(H, n, th)
    ...
    ants_per_batch = 5
    pr = 1
    ss = np.random.choice(np.arange(pr * ants_per_batch, pr * ants_per_batch + ants_per_batch), ants_per_batch,
                          replace=False)
    p = Process(target=ant_job_paral, args=(
    GE, N, H, th, 2, probs, a, b, cost, m, n, patients, count_big, cost_limit, L_g_min, L_g_max, G, ge, 5, 1, ss,))
    # p = Process(target = ant_job_paral, args = (GE,N,H,th,2,probs,a,b,cost,m,n,patients,count_big,cost_limit,L_g_min,L_g_max,G,ge,5,1,ss,result,))
    jobs.append(p)
    p.start()
    while np.abs(max_round_score - av_score) > eps and count_big < times and (W < m / 3):
...
def ant_job(GE, N, H, th, clusters, probs, a, b, cost, m, n, patients, count_big, cost_limit, L_g_min, L_g_max, G, ge,
            seed=None):
    paths = []
    wars = 0
...
def ants(a, b, n, m, H, GE, G, clusters, cost_limit, K, evaporation, th, L_g_min, L_g_max, eps, times, opt=None,
         pts=False, show_pher=True, show_plot=True, print_runs=True, save=None, show_nets=True):
    # fig = plt.figure(figsize=(10,8))
...
def ant_job(GE, N, H, th, clusters, probs, a, b, cost, m, n, patients, count_big, count_small, cost_limit, L_g_min,
            L_g_max, G, ge, print_runs):
    if print_runs:
