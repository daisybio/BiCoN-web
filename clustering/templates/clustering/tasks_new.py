...
@shared_task(name="algo_output_task_new")
def algo_output_task_new_2(s,L_g_min,L_g_max,expr_str,ppi_str,nbr_iter,nbr_ants,evap,epsilon,hi_sig,pher_sig,session_id,size,nbr_clusters):
	#col = "cancer_type"
	col = "disease_type"
	#size = 2000
	log2 = True
	...
	with open(("/code/clustering/static/output_console.txt"), "w") as text_file:
   		text_file.write("Your files are being processed...")
	if(nbr_clusters != 2):
		B,G,H,n,m,GE,A_g,labels_B,rev_labels_B = lib.aco_preprocessing_strings_2(expr_str, ppi_str, col,log2 = log2_2, gene_list = None, size = int(size), sample= None)
	else:
		B,G,H,n,m,GE,A_g,group1,group2,labels_B,rev_labels_B,val1,val2 = lib.aco_preprocessing_strings(expr_str, ppi_str, col,log2 = log2_2, gene_list = None, size = int(size), sample= None)
	#B,G,H,n,m,GE,A_g,group1,group2,labels_B,rev_labels_B,val1,val2 = lib.aco_preprocessing(fh, prot_fh, col,log2 = True, gene_list = None, size = size, sample= None)
	...
	# #GENERAL PARAMETERS:
	# =============================================================================
	clusters = nbr_clusters # other options are currently unavailable
	K = int(nbr_ants) # number of ants
	eps = float(epsilon) # stopping criteria: score_max-score_av<eps
	b = float(hi_sig) #HI significance
	evaporation  = float(evap)
